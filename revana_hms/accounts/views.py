...
class PasswordResetRequestView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        reset_link = (
            f"http://{current_site}/reset-password-confirm/?uid={uid}&token={token}"
        )
        ...
        html_message = render_to_string(
            "doctors/emails/password_reset_email.html", context
        )
        ...
        if user:
            ...
            try:
                ...
                if not user_obj.is_active:
                    ...
                    user_obj.role not in ["doctor", "hospital_admin"]
                    and not user_obj.is_superuser
                ):
                    messages.error(
                        request,
                        "Your account type is not authorized for portal access.",
                    )
                else:
    ...
    if request.method == "POST":
        ...
        if user.is_superuser:
            ...
            if user is not None:
                login(request, user)
                return JsonResponse(
                    {"success": True, "redirect_url": "/accounts/superadmin/dashboard/"}
                )
            else:
...
def superadmin_dashboard(request):
    ...
    doctor_monthly_qs = (
        User.objects.filter(role="doctor", date_joined__gte=twelve_months_ago)
...
def delete_user(request, user_id):
    if request.method == "POST":
        ...
        if hasattr(user, "hospitaladmin"):
            ...
            messages.success(
                request,
                f"Hospital {hospital.name} and Admin User deleted successfully.",
            )
        else:
            user.delete()
...
def manage_registrations(request):
    ...
    return render(
        request,
        "accounts/manage_registrations.html",
        {
            "pending_hospitals": pending_hospitals,
...
def superadmin_search(request):
    try:
        ...
        users = User.objects.filter(email__icontains=query).values(
            "id", "email", "role"
        )[:5]
...
def user_management(request):
    ...
    if tab == "doctors":
        queryset = (
            Doctor.objects.select_related("user", "hospital").all().order_by("-id")
        )
    else:
        queryset = (
            HospitalAdmin.objects.select_related("user", "hospital")
            .all()
            .order_by("-id")
        )
    ...
    if query:
        if tab == "doctors":
            ...
            queryset = queryset.filter(
                Q(user__email__icontains=query) | Q(hospital__name__icontains=query)
            )
...
@login_required
@user_passes_test(is_superadmin)
def mobile_advertisements(request):
    from advertisements.models import MobileAdvertisement

    advertisements = MobileAdvertisement.objects.all().order_by("-created_at")
    ...
    if request.method == "POST":
        ...
        if ad_id:
            ad = get_object_or_404(MobileAdvertisement, id=ad_id)
            if action == "delete":
                ad.delete()
                ...
                messages.success(
                    request,
                    f"Advertisement {'activated' if ad.is_active else 'deactivated'} successfully.",
                )

        return redirect("mobile_advertisements")

    context = {
        "advertisements": advertisements,
    }
    return render(request, "accounts/mobile_advertisements.html", context)


@login_required
@user_passes_test(is_superadmin)
def mobile_advertisement_create(request):
    from advertisements.models import MobileAdvertisement
    ...
    if request.method == "POST":
        ...
        ad = MobileAdvertisement.objects.create(
            title=title,
